...
public:
    ...
    //threadId_为EventLoop创建时的线程id CurrentThread::tid()为当前线程
...
private:
    ...
    //当wakeup()时，即有事件发生时，调用handleRead()读wakeupFd_的前8字节，同时唤醒阻塞的epoll_wait
    void handleRead();
    //执行上层回调
    ...
    //记录当前EventLoop是被哪个线程id创建的，即标识了当前的EventLoop的所属线程id
